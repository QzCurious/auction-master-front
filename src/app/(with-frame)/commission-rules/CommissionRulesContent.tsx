...
export function CommissionRulesContent({ rate }: { rate: number }) {
  return (
    <div className={docStyle.doc}>
      <ul
      ...
      >
        ...
        <li>
          ...
          <ol>
            ...
            <li>
              ...
              ：賣家自訂價格，物品由本平台上架至「日本雅虎拍賣」，以直購方式銷售。物品若售出，本平台將根據物品價格，扣除
              <span className={docStyle.red}>手續費{toPercent(rate)}</span>
              後支付款項予賣家，完成物品收購。
            </li>
            <li>
          ...
          <ul>
            <li>
              ...
              <ol className='pl-5'>
                ...
                <li>配合相關權利人進行法律追訴。</li>
                <li>
                  若違規行為導致本平台或相關第三方蒙受損失，將依法追究賠償責任。
                </li>
              </ol>
            </li>
          </ul>
        ...
        <li>
          ...
          <ul>
            ...
            <li>
              定價銷售與期約金額收購：待物品於「日本雅虎拍賣」完成結標後，本平台將扣除
              {toPercent(rate)}
              手續費後的拍賣所得款項撥至賣家的會員帳戶，賣家可隨時透過「帳戶紀錄」查詢並申請提領。
            </li>
            <li>
        ...
        <li>
          ...
          <ul>
            <li>
              本平台承諾提供「最低保障金額」。若拍賣結標金額低於本平台估值之最低價格，本平台將於拍賣後2至3週內，支付等同於最低估值扣除
              {toPercent(rate)}手續費之款項予賣家。
...
function CalcExample({
  ...
  return (
    <>
      例：若物品最低估值為
      {price}日圓，最終結標金額僅{endPrice}日圓，本平台仍將支付賣家
      {(1 - rate) * price}日圓（{price}× (1 - {toPercent(rate)})）。
    </>
  )
}
